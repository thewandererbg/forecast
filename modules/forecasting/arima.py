...
class AutoArimaForecaster(BaseForecaster):
    """AutoARIMA forecasting implementation using Prophet adapter with validation"""
    ...
    def __init__(
        ...
        # AutoARIMA Prophet adapter parameters
        self.arima_params = {
    ...
    def forecast_marketplace(
        ...
        """Generate forecast for a marketplace using AutoARIMA Prophet adapter + campaign calendar"""
        logger.debug(f"Forecasting {country}-{marketplace} (AutoARIMA + Campaigns)")
        ...
        # Initialize AutoARIMA Prophet adapter
        arima_params = self.arima_params.copy()
        model = AutoARIMAProphet(**arima_params)

        # Suppress warnings
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore")

            # Fit the model (Prophet-like interface)
            model.fit(dampened_data)

            # Generate forecast (Prophet-like interface)
        ...
        logger.debug(f"  AutoARIMA forecast total: {result['y_pred'].sum():.0f}")
